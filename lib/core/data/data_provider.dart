...
class DataProvider extends ChangeNotifier {
  ...
  Future<List<Category>> getAllCategories({bool showSnack = false}) async {
    try {
      ...
      if (response.isOk) {
        ...
        _filteredCategories =
            List.from(_allCategories); // Initiated filtered list with all data
        notifyListeners();
  ...
  // complete filterCategories // done
  ...
  // complete getAllSubCategory //done
  ...
  // complete filterSubCategories // done
  ...
  Future<void> getAllProducts({bool showSnack = false}) async {
    try {
      ...
      // i have added response.isOk here
      if (response.isOk) {
  ...
  Future<List<Coupon>> getAllCoupons({bool showSnack = false}) async {
    try {
      ...
      if (response.isOk) {
        ...
        _filteredCoupons =
            List.from(_allCoupons); // Initialize with original data
        notifyListeners();
        if (showSnack) SnackBarHelper.showSuccessSnackBar(apiResponse.message);
      }
    } catch (e) {
...
//? Filter products with quantity not equal to 0 or 1 (other stock)
  ...
  int calculateProductWithQuantity({int? quantity}) {
    int totalOrders = 0;
    // ? if target quantity is null it return total product
    if (quantity == null) {
      ...
      for (Product product in _allProducts) {
        if (product.quantity != null && product.quantity == quantity) {
          totalOrders +=
              1; // incrementing count if quantity meets or exceeds the target
        }
      }
    }
    return totalOrders;
